...
VIBEVOICE_BASE = "http://localhost:7860"
...
This chute boots the upstream `elbios/vibevoice-whisper:latest` image and proxies its
service endpoints via Chutes cords with JSON-to-multipart translation.

#### VibeVoice Endpoints (TTS - Text to Speech)
...
# -----------------------------------------------------------------------------
# VibeVoice Cords (port 7860)
# -----------------------------------------------------------------------------

@chute.cord(public_api_path="/api/generate_audio", public_api_method="POST")
async def generate_audio(self, args: dict) -> Any:
    """
    VibeVoice generate_audio endpoint.
    """
    has_base64 = any(k.endswith("_base64") for k in (args or {}).keys())
    if has_base64:
        return await _proxy_post_multipart(VIBEVOICE_BASE, "/api/generate_audio", args or {})
    return await _proxy_post_json(VIBEVOICE_BASE, "/api/generate_audio", args or {})
