...
CHUTE_BASE_IMAGE = "elbios/higgs-whisper:latest"
...
HIGGS_BASE = "http://localhost:7860"
...
This chute boots the upstream `elbios/higgs-whisper:latest` image and proxies its
service endpoints via Chutes cords with JSON-to-multipart translation.

#### Higgs Endpoints (TTS - Text to Speech)
...
image = (
    ...
    .set_user("root")
    .run_command("chmod -R a+rwx /opt/higgs 2>/dev/null || true")
    .set_user("chutes")
)
...
# -----------------------------------------------------------------------------
# Higgs Cords (port 7860)
# -----------------------------------------------------------------------------

@chute.cord(public_api_path="/api/generate_audio", public_api_method="POST")
async def generate_audio(self, args: dict) -> Any:
    """
    Higgs generate_audio endpoint.
    Expects JSON payload, proxied as multipart if any _base64 fields are present,
    otherwise as JSON. Gradio often prefers multipart for files.
    """
    has_base64 = any(k.endswith("_base64") for k in (args or {}).keys())
    if has_base64:
        return await _proxy_post_multipart(HIGGS_BASE, "/api/generate_audio", args or {})
    return await _proxy_post_json(HIGGS_BASE, "/api/generate_audio", args or {})
