...
- **XTTS + Whisper** (`deploy_xtts_whisper.py`)
  ...
  - Local `chutes run --dev` automatically spawns `elbios/xtts-whisper:latest` via Docker (with
    `--gpus all` or `--gpus $XTTS_DEV_GPUS`) if the entrypoint script isn’t on the host, so you can
    exercise the real vendor stack even off-container.
